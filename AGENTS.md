# AGENTS.md — agent-vibe-coding-starter-kit2-subway

## 프로젝트 개요
- 팀 주제: AI 기반 지하철 객차 혼잡도 실시간 분석 및 임산부석 안내 스마트 시스템
- 한 줄 목표: YOLO 기반 영상인식으로 객차 혼잡도를 분석하고 LED 및 웹 대시보드로 안내하며, 압력센서로 임산부석 점유 상태를 실시간 모니터링하는 시스템 구현
...
|---|---|
| 팀 주제 | AI 기반 지하철 객차 혼잡도 실시간 분석 및 임산부석 안내 스마트 시스템 |
| 액추에이터(제어 대상) 목록 | LED(LYG615C30 등 혼잡도 안내등 - 초록/주황/빨강), 컨베이어(모터 배경 이동) |
| 센서(모니터링 대상) 목록 | Pi Camera 3(영상 입력), 압력센서(임산부석 점유 감지) |
| 영상인식 감지 대상 | 객차 내 사람 감지(인원수 탐지 및 카운트) |
| 트리거 규칙 | 인원수에 따른 혼잡도 판별(여유/보통/혼잡) 시 LED 색상 변경 및 대시보드 반영, 압력센서 감지 시 임산부석 상태(사용 중/비어 있음) 실시간 갱신 |
