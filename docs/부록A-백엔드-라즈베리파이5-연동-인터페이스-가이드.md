...
> 📂 **부록A / 백엔드·DB 담당 + 라즈베리파이 담당 / 4단계 이후** · 전체 목록 [docs/README.md](README.md)
...
전체 목록은 `docs/부록B-iot-test-system-연동-가이드.md` 3장.
...
| gpiozero `BadPinFactory` / `lgpio` C 컴파일 에러 | Pi 5(RP1 칩)에 `lgpio` 미설치 또는 PyPI pip 빌드 실패 | `sudo apt install -y python3-gpiozero python3-lgpio` 설치 및 `python3 -m venv --system-site-packages venv` 가상환경 사용, `pi/.env`의 `GPIOZERO_PIN_FACTORY=lgpio` 확인 |
...
- **`docs/부록B-iot-test-system-연동-가이드.md`** — 라즈베리파이 담당자가 임시 테스트
